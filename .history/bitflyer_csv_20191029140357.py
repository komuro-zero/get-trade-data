...
class bitflyer_BTCJPY():
    ...
    def run(self,now,yesterday):
        ...
        while flag:
            if count == 0:
                executions = bitflyer_api.executions(product_code = product_code, count = 5000)
            else:
                executions = bitflyer_api.executions(product_code = product_code,before = before_id, count = 5000)
